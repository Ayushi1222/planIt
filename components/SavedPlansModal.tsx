...
export const SavedPlansModal: React.FC<SavedPlansModalProps> = ({ isOpen, onClose, plans, setPlans, activePlanId, setActivePlanId }) => {
    ...
    const handleSaveName = (planId: string) => {
        const newPlans = plans.map(p => p.id === planId ? { ...p, name: editingName } : p);
        setPlans(newPlans);
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-bkg-surface rounded-lg shadow-xl w-full max-w-lg border border-border-base flex flex-col max-h-[80vh]">
                ...
                <div className="p-6 overflow-y-auto space-y-3">
                    {plans.map(plan => (
                        <div key={plan.id} className={`flex items-center justify-between p-3 rounded-md transition-colors ${activePlanId === plan.id ? 'bg-primary/20' : 'bg-bkg-muted'}`}>
                            {editingPlanId === plan.id ? (
                                ...
                                <button onClick={() => handleSelectPlan(plan.id)} className="font-semibold text-left">
                                    {plan.name}
                                </button>
                            )}
